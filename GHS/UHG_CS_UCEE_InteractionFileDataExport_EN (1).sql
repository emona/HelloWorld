...
*/
	...
	SELECT @SQL = 'SELECT * from openquery(mtdata,''SELECT DISTINCT 
		...
		FROM TblJobs j WITH(NOLOCK)
			...
			LEFT JOIN tblPostalResp tpr
				ON s.TrackCode1 = tpr.TrackCode1
					AND tpr.datevalue >= '''''+CONVERT(varchar,@ReportStartDate,120)+'''''
					AND tpr.datevalue < '''''+CONVERT(varchar,@ReportEndDate,120)+'''''
		WHERE j.ClientID = 1857
			...
			AND p.Active = 1
			AND s.Active = 1
			AND p.MailDate >= '''''+CONVERT(varchar,@ReportStartDate-30,120)+''''' AND p.MailDate < '''''+CONVERT(varchar,@ReportEndDate,120)+''''''')'
